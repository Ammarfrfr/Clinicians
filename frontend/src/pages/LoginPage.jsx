...
export function LoginPage({ onNavigate, onLoginSuccess }) {
  ...
  return (
    <div className="min-h-screen bg-warm-white flex flex-col items-center justify-center p-6 text-center">
      ...
      <div className="w-full max-w-md bg-white border border-gray-200/60 rounded-2xl p-8 shadow-xs flex flex-col gap-4 text-left">
        <h2 className="text-xl font-bold text-navy text-left">Welcome Back</h2>
        <p className="text-xs text-gray-500 text-left -mt-2.5 mb-2">Sign in to your clinical note assistant</p>
        ...
        <button className="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-white border border-gray-200 hover:bg-gray-50 text-navy font-semibold text-sm rounded-xl transition-all cursor-pointer shadow-xs" onClick={handleGoogleLogin}>
        ...
        <div className="flex items-center w-full my-1.5">
          ...
          <span className="px-3 text-[11px] text-gray-400 uppercase font-bold tracking-wider">or sign in with email</span>
          <div className="flex-1 border-t border-gray-200/60"></div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <button type="submit" className="w-full inline-flex items-center justify-center px-4 py-3 bg-teal hover:bg-teal-dark text-navy font-semibold text-sm rounded-xl transition-all cursor-pointer border-none shadow-xs disabled:opacity-50 disabled:cursor-not-allowed mt-2" disabled={loading}>
            {loading ? (
        ...
        <p className="text-xs text-gray-500 mt-2 text-center">
          ...
          <span className="text-teal-dark hover:text-teal font-semibold cursor-pointer underline ml-0.5" onClick={() => onNavigate('register')}>
            Create one here
          </span>
        </p>
      </div>
